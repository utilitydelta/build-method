...
Awareness or performance is critical when writing code as it affects design decisions. And the code is the design.

New code without performance checks is not done.
...
- Performance is a sensor in the `implementation-loop`: ship the change, run the benchmark, read the number back.
- Performance tests are environment sensitive, baseline first.
- "noise is large" is not an explanation, it is a finding about the instrument. Fix it.
