...
description: The L0 map of the whole build method - how a goal travels from the human zone through the scout, into the sandboxed adversarial loop, and back via human replay into shippable code. Load at session start, or whenever unsure which zone the current work is in and which skill governs it. Every other skill is one level down from this one.
---

# Method Overview

Three zones. The human zone owns intent and the shippable code. The scout validates goals before they cost anything. The sandbox builds them autonomously. Code re-enters the human zone only through human replay. This file is the map; every stage links down to the skill that owns its detail.
